...
def test_repeat_roller():
    ...
    """
    errors = diff(Game().play, path="tests/assets/repeat_roller.sim.txt")
...
def test_hot_dice():
    ...
    """
    errors = diff(Game().play, path="tests/assets/hot_dice.sim.txt")
    assert not errors, errors[0]
...
def test_cheat_and_fix():
    ...
    errors = diff(Game().play, path="tests/assets/cheat_and_fix.sim.txt")
    assert not errors, errors[0]
...
def test_zilcher():
    ...
    errors = diff(Game().play, path="tests/assets/zilcher.sim.txt")
    assert not errors, errors[0]
